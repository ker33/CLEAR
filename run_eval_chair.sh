...
# ==========================================================
# ⚙️ 配置区：每次评测新模型，只需修改这里的变量！
# ==========================================================
# 实验名称 (最后保存的文件名)
EXP_NAME="llava-v1.5-7b-aaa"

# 模型权重路径
MODEL_PATH="./checkpoints/llava-v1.5-7b-dualcd-lora-mix"

# 官方基座模型路径 (如果不需要 base，删掉下面的 --model-base 行即可)
...
echo "=========================================================="
echo "🚀 开始一键执行 CHAIR 幻觉评测流程: $EXP_NAME"
echo "=========================================================="

# mkdir -p "$CHAIR_DIR/answers"

# # ---------------------------------------------------------
# # 第 0 步：检查环境并自动生成 500 道题的测试集 (如果没有的话)
# # ---------------------------------------------------------
...
# random.seed(42) # 固定随机种子，保证每次评测是同一批 500 张图
...
# ---------------------------------------------------------
# 第 1 步：运行 LLaVA 推理生成描述 (如果已跑完可注释掉这一段)
# ---------------------------------------------------------
...
# ---------------------------------------------------------
# 第 2 步：将 LLaVA 答案转换为 chair.py 需要的标准格式
# ---------------------------------------------------------
