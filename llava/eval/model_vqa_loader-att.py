...
def eval_model(args):
    ...
    apply_mask_guided_intervention(
        model=model, 
        start_layer=15, # 经验值：干预 15~32 层 (中高层负责具象语义，低层保留原始特征)
        end_layer=32, 
        threshold=1.0,  # 诊断阈值：当 看图得分 < 1.0 * 看字得分 时启动干预
        alpha=0.3,      # 靶向干预：物体区域的注意力增强系数
        b=0.1           # 靶向干预：历史文本的注意力抑制系数
    )
    # ==========================================================
    ...
    for (input_ids, image_tensor, image_sizes), line in tqdm(zip(data_loader, questions), total=len(questions)):
        ...
        # ==========================================================
        # 🌟 创新注入 3：动态定位当前数据的 <image> 位置
        # ==========================================================
        # 找到输入序列中 IMAGE_TOKEN_INDEX 的位置
        ...
        if len(image_token_pos) > 0:
            ...
            # 文本包含图片前和图片后(被往后挤了575个位置)的所有长度
            text_end_idx = input_ids.shape[1] + 575 
            ...
            for i in range(len(model.model.layers)):
                if hasattr(model.model.layers[i].self_attn, 'use_intervention'):
                    ...
                    attn.text_start_idx = 0
                    attn.text_end_idx = text_end_idx
        # ==========================================================
